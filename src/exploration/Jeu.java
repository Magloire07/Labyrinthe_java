...
public class Jeu
{
    ...
    public void joue()
    {
        ...
        }while(!(getJoueur().isGagnant()||getJoueur().isPerdant())); // on boucle tand que le joueur  n'est ni gagnant ni perdant 
    ...
    public Jeu(int nbLig, int nbCol, int nbPierres,Categorie... liste)
    {
     ...
     //la joueur est ajouté au jeu  avec son nom qui est demander grace à la classe Lire.java 
     //la case à numlig=1 et numCol=0 lui est ajouté comme case de départ, cette case etant récupéré depuis la plateau
